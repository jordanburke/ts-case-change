...
export function toSnake<T extends string>(term: T): ToSnake<T> {
  ...
  while ((/(.+?)([A-Z])/.exec(result)?.length || 0) > 2 && circuitBreaker < 10) {
    result = result.replace(/(.+?)([A-Z])/, (_all, $1: string, $2: string) => `${$1.toLowerCase()}_${$2.toLowerCase()}`)
    circuitBreaker += 1
  }
...
function convertObjectSavePrefix<T extends object>(obj: T): ObjectToCamelPrefix<T> {
  if (obj === null || typeof obj === "undefined" || typeof obj !== "object") {
